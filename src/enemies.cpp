...
void Firebeams::draw(glm::mat4 VP) {
    Matrices.model = glm::mat4(1.0f);
    glm::mat4 translate = glm::translate (this->position);    // glTranslatef
    glm::mat4 scale = glm::scale (glm::vec3(3.5, 0.9, 1));    // glScalef
    glm::mat4 rotate    = glm::rotate((float) (this->rotation * M_PI / 180.0f), glm::vec3(0, 0, 1));
    // No need as coords centered at 0, 0, 0 of cube arouund which we waant to rotate
    // rotate          = rotate * glm::translate(glm::vec3(0, -0.6, 0));
    Matrices.model *= (translate * rotate*scale);
...
void Firebeams::tick(Player* hooman){
    float facto=0.10f;
    if(this->position.y > 6) facto = 0.10f;
    if(this->position.y < hooman->position.y)  facto = -0.10f;
    this->position.y -= facto;
    ...
    if(detect_collision(side1,b))
    {
        ...
        hooman->position.x = side1.x-1;
        hooman->position.y = tempy+1;
    }
